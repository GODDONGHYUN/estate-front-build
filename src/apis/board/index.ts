...
export const postBoardRequest = async (
  ...
  const result = await axios
    .post(POST_BOARD_REQUEST_URL, requestBody, bearerAuthorization(accessToken))
...
export const postCommentRequest = async (
  ...
  const result = await axios
    .post(
      POST_COMMENT_REQUEST_URL(receptionNumber),
      requestBody,
...
export const getBoardListRequest = async (accessToken: string) => {
  const result = await axios
    .get(GET_BOARD_LIST_URL, bearerAuthorization(accessToken))
...
export const getSearchBoardListRequest = async (
  ...
  const result = await axios
    .get(GET_SEARCH_BOARD_LIST_URL, config)
    .then(requestHandler<GetSearchBoardResponseDto>)
    .catch(requestErrorHandler);
  return result;
};
...
export const increaseViewCountRequest = async (
  ...
  const result = await axios
    .patch(
      INCREASE_VIEW_COUNT_URL(receptionNumber),
      {},
      bearerAuthorization(accessToken)
    )
    .then(requestHandler<ResponseDto>)
