...
export default function QnaDetail() {
  ...
  const getBoardResponse = (
    result: GetBoardResponseDto | ResponseDto | null
  ) => {
    const message = !result
      ? "서버에 문제가 있습니다."
      : result.code === "VF"
  ...
  const onDeleteClickHandler = () => {
    ...
    deleteBoardRequest(receptionNumber, cookies.accessToken).then(
      deleteBoardResponse
    );
  };
  ...
  useEffect(() => {
    if (!cookies.accessToken || !receptionNumber) return;
    increaseViewCountRequest(receptionNumber, cookies.accessToken).then(
  ...
  const coverdWriterId =
    writerId !== "" && writerId[0] + "*".repeat(writerId.length - 1);
  return (
    <div id="qna-detail-wrapper">
      ...
      <div className="qna-detail-button-box">
        <div className="primary-button" onClick={onListClickHandler}>
          목록보기
        </div>
        {loginUserId === writerId && (
          <div className="qna-detail-owner-button-box">
            {!status && (
              <div className="second-button" onClick={onUpdateClickHandler}>
                수정
              </div>
            )}
            <div className="error-button" onClick={onDeleteClickHandler}>
              삭제
            </div>
          </div>
        )}
