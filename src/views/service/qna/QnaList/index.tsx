...
} from "src/apis/board/dto/response";
...
export default function QnaList() {
  ...
  const getBoardListResponse = (
    result: GetBoardListResponseDto | ResponseDto | null
  ) => {
  ...
  const getSearchBoardListResponse = (
    result: GetSearchBoardResponseDto | ResponseDto | null
  ) => {
    const message = !result
      ? "서버에 문제가 있습니다."
      : result.code === "VF"
  ...
  const onSearchButtonClickHandler = () => {
    ...
    getSearchBoardListRequest(searchWord, cookies.accessToken).then(
    ...
    <div id="qna-list-wrapper">
      <div className="qna-list-top">
        <div className="qna-list-size-text">
          전체 <span className="emphasis">{totalLenght}건</span> | 페이지{" "}
